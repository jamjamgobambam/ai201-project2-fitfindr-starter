...
def _parse_query(query: str) -> dict:
    ...
    Uses regex rather than an LLM call so parsing is deterministic and testable
    (documented choice in planning.md). The price/size clauses are stripped from
    the description so they don't add noise to keyword scoring.
    ...
    # max_price: "under $30", "below 30", "less than $30.50", "< 30", "$30 max"
    ...
    # size: "size M", "size XXS", "in a size 8"
    size_match = re.search(r"\bsize\s+([A-Za-z0-9/]+)", query, re.IGNORECASE)
    if size_match:
        size = size_match.group(1).strip()
        description = description.replace(size_match.group(0), " ")

    # Clean up punctuation/whitespace orphaned by the removed clauses.
    description = re.sub(r"\s+", " ", description)
    description = re.sub(r"\s+([,.;])", r"\1", description)  # un-orphan punctuation
...
def run_agent(query: str, wardrobe: dict) -> dict:
    ...
    if not session["search_results"]:
        # No matches — end early with a helpful message, do NOT call suggest_outfit.
        hints = []
        if parsed["max_price"] is not None:
            hints.append("raising your price limit")
        if parsed["size"]:
            hints.append("removing the size filter")
        hints.append("trying different keywords")
        session["error"] = (
            "No listings matched your search. Try " + ", or ".join(hints) + "."
        )
        return session
