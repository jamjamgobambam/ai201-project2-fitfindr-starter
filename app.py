...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    if not user_query or not user_query.strip():
        return "Please enter a search query (e.g., 'vintage graphic tee').", "", ""

    # 2. Select the wardrobe based on the radio button choice
    ...
    # 4. Handle the error branch (if search_listings failed)
    if session.get("error"):
        return session["error"], "", ""

    # 5. Format the success output
    item = session["selected_item"]
    
    # Create a clean, readable string for the first Gradio panel
    listing_text = (
